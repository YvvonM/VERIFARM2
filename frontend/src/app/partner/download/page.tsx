...
export default function PartnerDownloadPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      <PortalNav />
      <h1 className="text-2xl font-semibold text-white">Data Download</h1>
      <p className="mt-1 text-sm text-white/60">
        Export verified claims matching your filters.
      </p>
      ...
      {claims.length > 0 ? (
        <div className="mt-6 overflow-x-auto rounded-md border border-white/10">
          <p className="border-b border-white/10 px-3 py-2 text-xs text-white/40">
            {claims.length} matching claim(s) — showing first 10.
          </p>
          <table className="w-full text-left text-sm">
            <thead className="bg-white/5">
              <tr>
                <th className="px-3 py-2 font-medium text-white/60">Farmer ID</th>
                <th className="px-3 py-2 font-medium text-white/60">Claim type</th>
                <th className="px-3 py-2 font-medium text-white/60">Value</th>
                <th className="px-3 py-2 font-medium text-white/60">Confidence</th>
                <th className="px-3 py-2 font-medium text-white/60">Attested by</th>
                <th className="px-3 py-2 font-medium text-white/60">Authoritative</th>
              </tr>
            </thead>
            <tbody>
              {claims.slice(0, 10).map((c, i) => (
                <tr key={c.claim_id ?? i} className="border-t border-white/10">
                  ...
                  <td className="px-3 py-2 text-white/80">{c.claim_type}</td>
      ...
      <div className="mt-8 rounded-lg border-l-4 border-secondary bg-secondary/10 p-4 text-sm text-secondary">
        Downloaded data includes verified claims only. Raw farmer records are never exported.
        Usage is governed by your partner agreement.
      </div>
    </main>
  );
}
